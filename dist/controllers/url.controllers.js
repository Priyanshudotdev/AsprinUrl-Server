...
export const handleFetchUserSingleLink = async (req, res) => {
    ...
    if (myCache.has(`${userId}-${urlTitle}`)) {
        link = JSON.parse(myCache.get(`${userId}-${urlTitle}`));
    }
    else {
        link = await Url.find({
            createdBy: userId,
...
export const handleFetchUserLinks = async (req, res) => {
...
export const handleDeleteLink = async (req, res) => {
